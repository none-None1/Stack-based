...
def stack_based(c,timeout=None):
    ...
    def xor(cmd,ip):
        ...
        varl[q[2]]=q[0]^q[1]
        return ip+1
    cmddict={'var':var,'i':i,'o':o,'ic':ic,'oc':oc,'p':p,'s':s,'a':a,'su':su,'m':m,'q':Q,'r':R,'c':C,'push':push,'pop':pop,'top':top,'sz':sz,'lt':lt,'gt':gt,'le':le,'ge':ge,'eq':eq,'ne':ne,'ja':ja,'jb':jb,'nop':nop,'halt':halt,'rnd':rnd,'and':And,'or':Or,'not':Not,'bnd':bnd,'bor':bor,'xor':xor}
    if timeout:
        starttime=time.time()
    while 0 <= ip < len(cmds):
        if timeout:
